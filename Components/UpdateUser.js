...
function UpdateUser({ itemData }) {
  ...
  const validate = (values) => {
    console.log(values,"value here this")
    const errors = {};
    ...
    if (!values?.name) {
      ...
      errors.name = "Name should be greater than 2 and less than 20 character";
    }
    if (!values?.contactNo) {
  ...
  const getUserById = async () => {
    try {
      ...
      const response = await axios.post("/api/getUserById", {
        token: token,
        userId: `${itemData.id}`,
      });
  ...
  const updateUserById = async (data) => {
    try {
      console.log(data,"api data here for update")
      const token = localStorage.getItem("token");
                ...
                >
                  {rollName?.map((item) => {
                    return (
                      <>
                        <option
                          selected={item?.id == formData?.roleId}
                          value={item?.id}
                        >
                          {" "}
                          {item?.roleName}
