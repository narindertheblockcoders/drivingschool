...
const EditClientDetailModal = ({
  ...
  const getSwapData = async (newData) => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.post("/api/EditClientBooking", {
        token: token,
        data: newData,
      });
      ...
      >
        ...
        <Modal.Body>
          ...
          <div className="input-main">
            ...
            <div className="sclient-box">
              <div className="sclient-div mb-3 "  aria-label="Default select example">
                <select
                  className="form-select"
                  id="book-select"
                  name="vehicleTypeId"
